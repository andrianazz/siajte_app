...
class EditJadwalKpController extends GetxController {
  ...
  Future<Map<String, dynamic>> editJadwalKPAPI() async {
    ...
    debugPrint(penjadwalanKp.id.toString());
    debugPrint(mahasiswa.nim.toString());
    debugPrint(pembimbing.nip.toString());
    debugPrint(penguji.nip.toString());
    debugPrint(prodiId.toString());
    debugPrint(judulKP.text);
    debugPrint(tanggal.value.toString().substring(0, 10));
    debugPrint("${waktu.value.hour}:${waktu.value.minute}:00");
    debugPrint(lokasi.text);
    debugPrint(mapUser['data']['id'].toString());
